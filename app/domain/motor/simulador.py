...
def _preferencia_modalidad(modalidad: str | None, monto: float, liquidez: float) -> int:
    """
    DESEMPATE con regla de negocio: si la liquidez supera los 2,000, 
    se ajusta la preferencia hacia el uso de tarjetas de crédito o MSI 
    para proteger el efectivo.
    """
    if not modalidad:
        return 3

    # Regla nueva: Si hay más de 2,000 de liquidez, priorizamos tarjeta/MSI sobre contado
    if liquidez > 2000:
        if modalidad == "credito" or "_msi" in modalidad:
